...
def compute_risk_reward(
    ...
    try:
        ...
        return None
    if not all(math.isfinite(value) for value in (e, tp, sl)):
        return None
    ...
    if not math.isfinite(risk) or not math.isfinite(reward):
        return None
...
def validate_take_profit_2_geometry(
    ...
    if not all(math.isfinite(value) for value in (e, t1, t2, s)):
        return ["decision.take_profit_price_2: required finite number when placing an order"]
