...
def _find_non_finite_numbers(value: Any, path: str = "$") -> list[str]:
    """Return JSON paths containing NaN or positive/negative infinity."""
    if isinstance(value, float) and not math.isfinite(value):
        return [path]
    if isinstance(value, dict):
        invalid: list[str] = []
        for key, child in value.items():
            invalid.extend(_find_non_finite_numbers(child, f"{path}.{key}"))
        return invalid
    if isinstance(value, list):
        invalid = []
        for index, child in enumerate(value):
            invalid.extend(_find_non_finite_numbers(child, f"{path}[{index}]"))
        return invalid
    return []
...
class JsonValidator:
    ...
    def validate(
        ...
        *,
        ...
        non_finite = _find_non_finite_numbers(obj)
        if non_finite:
            return ValidationError(
                category="c",
                stage=stage,
                raw_text=raw_text,
                invalid_fields=non_finite,
                message="Non-finite numeric values are not allowed",
            )
        ...
        if non_finite:
            return ValidationError(
                ...
                raw_text=raw_text,
                invalid_fields=non_finite,
                message="Normalization produced a non-finite numeric value",
            )
        norm_mode = getattr(self._validation, "normalization_mode", "strict")
        ...
        try:
            ...
            logger.error("jsonschema not installed; refusing unvalidated model output")
            return ValidationError(
                category="c",
                stage=stage,
                raw_text=raw_text,
                invalid_fields=["validator:jsonschema_unavailable"],
                message="jsonschema is required for hardened output validation",
            )
