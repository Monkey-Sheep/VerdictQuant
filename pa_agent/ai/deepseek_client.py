...
# KKAI's OpenAI-compatible route accepts a six-digit Anthropic thinking budget.
...
def _resolve_thinking_params(
    ...
    *,
    ...
    # Gateway protocol wins over the model name. KKAI may expose a DeepSeek-like
    # model id while requiring Anthropic-style thinking budgets.
    if _is_kkai_openai_proxy(settings.base_url):
        if not _thinking:
            return {}, None
    ...
    if _is_deepseek_native(settings.base_url) or _is_deepseek_model(model):
        # DeepSeek OpenAI format uses thinking.type=enabled/disabled and a
        # top-level reasoning_effort (high/max).
        if _thinking:
            return {"thinking": {"type": "enabled"}}, _effort or "high"
...
class DeepSeekClient:
    ...
    def update_provider(self, settings: AIProviderSettings) -> None:
        """Replace in-memory provider settings after enforcing hardened policy."""
        from pa_agent.security.policy import ensure_provider_allowed
