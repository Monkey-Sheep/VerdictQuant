...
def validate_bar_by_bar_vs_features(
    ...
    *,
    ...
    for i, item in enumerate(summary):
        ...
        # Structural geometry remains authoritative in strict mode, while
        # threshold-sensitive trend/doji differences stay tolerant near cutoffs.
        _ALL_BAR_TYPES = _STRUCTURAL_TYPES | _THRESHOLD_SENSITIVE_TYPES
        if declared in _ALL_BAR_TYPES and computed in _ALL_BAR_TYPES:
            pair = (declared, computed)
            ...
            structural_mismatch = strict and (
                declared in _STRUCTURAL_TYPES or computed in _STRUCTURAL_TYPES
            )
            directional_mismatch = pair in _opposites or pair[::-1] in _opposites
            if structural_mismatch or directional_mismatch:
                errors.append(
