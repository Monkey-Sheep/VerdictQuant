...
def _repair_gate_result(obj: dict[str, Any]) -> None:
    ...
    if not isinstance(gate, list) or not gate:
        return
    # A synthetic AUTO node means the JSON tail was reconstructed after a
    # truncated model response.  Keep that result fail-closed: the missing
    # gates were never actually evaluated, so ``unknown`` cannot become
    # ``proceed`` merely because no explicit blocker survived truncation.
    if any(
        isinstance(item, dict)
        and str(item.get("node_id", "")).strip().upper() == "AUTO"
        for item in gate
    ):
        return
