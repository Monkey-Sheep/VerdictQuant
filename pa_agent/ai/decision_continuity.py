...
def _parse_local_iso_ms(iso: str | None, *, current_ms: int | None = None) -> int | None:
    ...
    try:
        ...
        local_ms = int(parsed.timestamp() * 1000)
        utc_wall_ms = int(calendar.timegm(parsed.utctimetuple()) * 1000 + parsed.microsecond / 1000)
        if current_ms is None or local_ms == utc_wall_ms:
            return local_ms
        candidates = (local_ms, utc_wall_ms)
        past = [candidate for candidate in candidates if candidate <= int(current_ms)]
        return min(past or candidates, key=lambda candidate: abs(int(current_ms) - candidate))
...
def bars_elapsed_between(
    ...
    *,
    ...
    if not prev_time_iso or not current_ms:
        return fallback
    prev_ms = _parse_local_iso_ms(prev_time_iso, current_ms=current_ms)
    if prev_ms is None:
        return fallback
...
def _assess_pending_plan_close_invalidation(
    decision: dict | None,
    frame: Any,
) -> tuple[bool, str]:
    """Cancel an unfilled plan once the latest close is beyond its stop.

    A pre-entry wick through the stop does not imply a fill and stop-out. A
    close beyond the stop, however, means the setup's structural premise is no
    longer valid and the resting order must not be carried forward.
    """
    if not is_order_plan(decision):
        return False, ""
    sign = order_direction_sign(str((decision or {}).get("order_direction") or ""))
    stop = _parse_price((decision or {}).get("stop_loss_price"))
    bars = list(getattr(frame, "bars", ()) or ())
    if not bars or stop is None or sign == 0:
        return False, ""

    close = float(getattr(bars[0], "close", 0))
    if sign > 0 and close <= stop:
        return True, f"K1 收盘 {close} 跌破止损 {stop}（未成交做多计划取消）"
    if sign < 0 and close >= stop:
        return True, f"K1 收盘 {close} 突破止损 {stop}（未成交做空计划取消）"
    return False, ""
...
def assess_limit_order_triggered(
    ...
    *,
    ...
    for bar in scan:
        ...
        touched = low - tol <= entry <= high + tol
        if order_type == "限价单" and touched:
            return (
                True,
                f"K{seq} range=[{low}, {high}] 已触及限价入场 {entry}",
                seq,
            )
        if order_type == "突破单" and sign > 0 and high >= entry - tol:
            return (
                True,
                f"K{seq} high={high} 已触及/突破买入触发价 {entry}",
                seq,
            )
        if order_type == "突破单" and sign < 0 and low <= entry + tol:
            return (
                True,
                f"K{seq} low={low} 已触及/跌破卖出触发价 {entry}",
                seq,
            )
    return False, "", None
...
def assess_combined_plan_invalidation(
    ...
    *,
    ...
    pending_entry = order_type in ("限价单", "突破单") and not limit_triggered
    if pending_entry:
        invalidated, invalidation_reason = _assess_pending_plan_close_invalidation(
            prev_decision,
            frame,
        )
    else:
        invalidated, invalidation_reason = assess_plan_invalidation(prev_decision, frame)
    auto_invalidated, auto_reason = assess_unfilled_limit_auto_cancel(
        prev_decision=prev_decision,
        prev_stage1=prev_stage1,
        curr_stage1=curr_stage1,
        bars_since=bars_since,
        # Pending-close invalidation and regime/age cancellation are independent
        # facts. Evaluate both so the audit reason does not hide one of them.
        invalidated=False if pending_entry else invalidated,
        limit_triggered=limit_triggered,
        max_pending_bars=max_pending_bars,
    )
    invalidated = invalidated or auto_invalidated
    if auto_reason:
        invalidation_reason = (
            auto_reason
...
def audit_relation_fields(
    ...
    *,
    ...
    # CSV audit rows do not carry the prior Stage-1 regime. Keep this label about
    # directional relation and hard stop invalidation; age/regime expiry belongs
    # to build_continuity_context where the required context is available.
