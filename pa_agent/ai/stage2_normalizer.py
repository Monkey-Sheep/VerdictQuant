...
def _normalize_stage2_enum_aliases(out: dict[str, Any]) -> bool:
    ...
    if _normalize_stage2_bar_analysis_enums(out):
        changed = True
...
def _coerce_decision_when_trade_metrics_fail(
    ...
    *,
    ...
    if not isinstance(decision, dict) or decision.get("order_type") not in _TRADE_ORDER_TYPES:
        return False
    # Incomplete payloads must remain available to the other normalizers. Fully
    # specified model-only payloads still need the same deterministic RR checks
    # used by JsonValidator, even when no market frame was supplied.
    required_prices = (
        "entry_price",
        "stop_loss_price",
        "take_profit_price",
    ...
    if any(decision.get(field) is None for field in required_prices):
        return False
...
def _normalize_next_bar_prediction(prediction: dict[str, Any]) -> None:
    ...
    if isinstance(probs, dict):
        ...
        # 5. direction = argmax (R3.3), with a deterministic literal-order tie-break.
...
def normalize_stage2(
    ...
    *,
    ...
    if isinstance(decision, dict) and decision.get("order_type") == "不下单":
        # Do not erase non-null order fields here.  They are a semantic model
        # error and must reach the schema/cross-field validator.  Decisions
        # deliberately coerced to no-order were already cleared by
    ...
    # Run the final trade-metric gate after signal-chain repairs and the
    # directional prediction guard. A prediction conflict intentionally wins
    # with a terminal ``wait``; otherwise complete invalid prices are rejected.
