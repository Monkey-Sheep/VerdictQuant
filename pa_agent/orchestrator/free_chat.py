...
class FreeChatSession:
    ...
    def __init__(
        ...
        # These two messages are byte-for-byte identical across every turn of
    ...
    def _build_prefix(base_record: AnalysisRecord) -> list[dict]:
        ...
        prefix.append(
            {
                "role": "system",
                "content": (
                    "你是 VerdictQuant 的【追问助手】（post-analysis advisor），不是在执行新的完整两阶段分析。\n"
        ...
        # Add a reliable recall summary from the *parsed*
        ...
        # Build a concise, factual recall from validated fields only. It remains
        # part of the user-owned reference message so it cannot be mistaken for
        # a prior assistant turn when reasoning replay is enabled.
