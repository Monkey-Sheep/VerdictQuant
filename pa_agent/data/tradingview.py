...
from pa_agent.data.tradingview_proxy import configure_tvdatafeed_proxy
...
class TradingViewSource(DataSource):
    ...
    def connect(self) -> None:
        try:
            ...
            proxy = configure_tvdatafeed_proxy()
            ...
            logger.info(
                "TradingViewSource connected (anonymous=%s, proxy=%s)",
                not self._username,
                proxy.display if proxy else "direct",
            )
...
def _row_ts_ms(row) -> int:
    """Extract a tvDatafeed local-wall-clock timestamp as UTC milliseconds."""
    value = getattr(row, "datetime", None)
    try:
        from datetime import datetime

        if isinstance(value, datetime) and value.tzinfo is None:
            to_python = getattr(value, "to_pydatetime", None)
            local_value = to_python() if callable(to_python) else value
            return int(naive_local_to_utc(local_value).timestamp() * 1000)
    except (TypeError, ValueError, OverflowError):
        pass
    return datetime_to_ts_ms(value)
