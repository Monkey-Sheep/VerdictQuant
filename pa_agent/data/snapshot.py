...
def _has_forming_head(
    bars_raw: list[KlineBar],
    timeframe: str,
    symbol: str,
    now_ms: int | None,
) -> bool:
    """Use the feed flag unless an authoritative clock was supplied."""
    if not bars_raw:
        return False
    if now_ms is None:
        return not bars_raw[0].closed
...
def _newest_closed_slice(
    ...
    *,
    ...
    if not bars_raw or n < 1:
        return None
    forming = _has_forming_head(bars_raw, timeframe, symbol, now_ms)
...
def build_live_frame(
    ...
    *,
    ...
    """
    has_forming = _has_forming_head(bars_raw, timeframe, symbol, now_ms)
    if has_forming:
