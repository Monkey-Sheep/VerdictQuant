"""Main application window for VerdictQuant."""
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, ctx: AppContext, parent: QWidget | None = None) -> None:
        super().__init__(parent)
        self.setWindowTitle("VerdictQuant — AI研究与模拟量化工作台（不构成投资建议）")
        ...
        self._startup_update_result_check_done = False
        ...
        self._setup_ui()
        from pa_agent.gui.update_controller import UpdateController

        self._update_controller = UpdateController(self)
        self._update_controller.completed.connect(self._on_update_completed)
        self._update_controller.failed.connect(self._on_update_failed)
        self._update_controller.progress.connect(self._on_update_progress)
        self._update_controller.activity.connect(self._on_update_activity)
    ...
    def _setup_ui(self) -> None:
        ...
        self._main_tabs.addTab(self._analysis_workbench, "PA K线分析")
        self._finance_hub = FinanceHubWidget(self)
        self._finance_hub.symbol_selected.connect(self._on_finance_symbol_selected)
        self._main_tabs.addTab(self._finance_hub, "综合研究中心")
        self._central = self._main_tabs
        ...
        _finance_action = QAction("综合研究中心", self)
        _finance_action.triggered.connect(lambda: self._main_tabs.setCurrentIndex(1))
        menu_bar.addAction(_finance_action)
        ...
        self._check_update_action = QAction("检查更新", self)
        self._check_update_action.triggered.connect(lambda: self._check_for_updates(manual=True))
        help_menu.addAction(self._check_update_action)
        self._update_settings_action = QAction("更新设置", self)
        self._update_settings_action.triggered.connect(self._open_update_settings_dialog)
        help_menu.addAction(self._update_settings_action)
    ...
    def _build_workbench(self) -> QWidget:
        ...
        self._data_source_combo.setToolTip(
            "加固版默认使用 TradingView；MT5 仅在你主动选择时初始化本机终端。"
        )
    ...
    def _on_finance_symbol_selected(self, market: str, symbol: str, exchange: str) -> None:
        """Load a research-center symbol into PA without starting network work."""
        source_index = self._data_source_combo.findData("tradingview")
        if source_index >= 0:
            self._data_source_combo.setCurrentIndex(source_index)
        exchange_index = self._tv_exchange_combo.findData(exchange)
        if exchange_index < 0:
            exchange_index = self._tv_exchange_combo.findData("")
        if exchange_index >= 0:
            self._tv_exchange_combo.setCurrentIndex(exchange_index)
        self._symbol_combo.setCurrentText(symbol)
        self._tf_combo.setCurrentText("5m" if market == "crypto" else "1d")
        self._main_tabs.setCurrentIndex(0)
        label = {"a-share": "A股", "crypto": "加密资产"}.get(market, "美股")
        self._status_bar.showMessage(
            f"已从综合研究中心载入 {label} {symbol}；点击「获取数据」后再提交PA分析"
        )
    ...
    def _connect_current_data_source(self) -> bool:
        """Connect the selected source after an explicit user action."""
        data_source = getattr(self._ctx, "data_source", None)
        if data_source is None:
            self._status_bar.showMessage("数据源不可用")
            return False
        if getattr(data_source, "_connected", False):
            return True
        try:
            data_source.connect()
            self._apply_tv_exchange_to_source(data_source)
        except Exception as exc:  # noqa: BLE001
            logger.warning("On-demand data source connection failed: %s", exc)
            self._disconnect_data_source(data_source)
            self._status_bar.showMessage(f"数据源连接失败：{exc}")
            return False
        self._status_bar.showMessage("数据源已连接，正在准备行情…")
        return True
    ...
    def _on_fetch_data_clicked(self) -> None:
        ...
        if self._current_data_source_kind() == "tradingview":
            ...
            if not ok:
                ...
                choice = show_tv_connectivity_blocked_dialog(self, detail=detail)
                if choice == "mt5":
            ...
            _time.sleep(1.5)
        if not self._connect_current_data_source():
            return
    ...
    def _maybe_show_truncation_help_dialog(self, exc_info: dict | None) -> None:
        ...
        box.setText("本次分析的 JSON 正文可能因「模型上下文/输出额度不足」而被截断，导致校验失败。")
    ...
    def _on_record_ready_impl(self, record: Any) -> None:
        ...
        if panel is not None:
            ...
            if s1_diag:
                ...
                s1_reasoning = reasoning_from_response(s1_raw if isinstance(s1_raw, dict) else None)
            ...
            if s2_decision:
                ...
                s2_reasoning = reasoning_from_response(s2_raw if isinstance(s2_raw, dict) else None)
    ...
    def closeEvent(self, event: QCloseEvent | None) -> None:
        ...
        try:
            ...
            if flow_viz is not None:
                flow_viz.shutdown()
            finance_hub = getattr(self, "_finance_hub", None)
            if finance_hub is not None:
                finance_hub.shutdown()
            update_controller = getattr(self, "_update_controller", None)
            if update_controller is not None:
                update_controller.cancel()
        except RuntimeError as exc:
            logger.debug("Shutdown cleanup skipped: %s", exc)
        super().closeEvent(event)

    def showEvent(self, event: QShowEvent | None) -> None:
        """Refresh non-blocking startup status on first show."""
        ...
        if not self._startup_update_result_check_done:
            self._startup_update_result_check_done = True
    ...
    def _on_startup_update_check(self) -> None:
        settings = getattr(self._ctx, "settings", None)
        if settings is None or not settings.updates.auto_check:
            return
    ...
    def _check_for_updates(self, *, manual: bool) -> None:
        ...
        from pa_agent.brand import PRODUCT_VERSION
        from pa_agent.update.github_release import update_token_from_environment

        settings = getattr(self._ctx, "settings", None)
        update_settings = getattr(settings, "updates", None)
        configured_token = getattr(update_settings, "github_token", "") if update_settings else ""
        token = str(configured_token or update_token_from_environment()).strip()
        auto_download = bool(
            (getattr(update_settings, "auto_download", True) or manual)
            and getattr(sys, "frozen", False)
        )
        started = self._update_controller.start(
            current_version=PRODUCT_VERSION,
            token=token,
            manual=manual,
            auto_download=auto_download,
        )
        if not started and manual:
            self._status_bar.showMessage("更新检查正在运行")

    def _on_update_activity(self, active: bool) -> None:
        action = getattr(self, "_check_update_action", None)
        if action is not None:
            action.setEnabled(not active)
        if active:
            self._status_bar.showMessage("正在检查更新...")

    def _on_update_progress(self, percentage: int) -> None:
        self._status_bar.showMessage(f"正在下载更新... {percentage}%")

    def _on_update_failed(self, manual: bool, message: str) -> None:
        logger.warning("Update check failed: %s", message)
        if manual:
            QMessageBox.warning(self, "更新检查失败", message)
        else:
            self._status_bar.showMessage(
                "自动更新检查失败；可在“帮助 → 更新设置”中检查网络或私仓令牌",
                10000,
            )

    def _on_update_completed(self, outcome: object) -> None:
        from pa_agent.gui.update_controller import UpdateOutcome

        if not isinstance(outcome, UpdateOutcome):
            return
        release = outcome.release
        if release is None:
            self._status_bar.showMessage("当前已经是最新版本", 5000)
            if outcome.manual:
                QMessageBox.information(self, "检查更新", "当前已经是最新版本。")
            return
        if outcome.package_path is None:
            self._status_bar.showMessage(f"发现新版本 {release.tag_name}", 8000)
            if outcome.manual:
                QMessageBox.information(
                    self,
                    "发现新版本",
                    f"已发现 {release.tag_name}。自动安装仅在 Windows 打包版中可用。",
                )
            return
        ...
        answer = QMessageBox.question(
            self,
            "更新已下载",
            f"VerdictQuant {release.tag_name} 已下载并通过 SHA-256 校验。\n\n"
            "现在退出程序、安装更新并自动重启吗？",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.Yes,
        )
        if answer != QMessageBox.StandardButton.Yes:
            self._status_bar.showMessage(f"{release.tag_name} 已下载，可稍后再次检查并安装", 8000)
            return
        try:
            from pa_agent.update.launcher import launch_update_installer

            launch_update_installer(release, outcome.package_path)
        except Exception as exc:  # noqa: BLE001
            logger.exception("Unable to launch update installer")
            ...
            return
        self._status_bar.showMessage("正在退出并安装更新...")
        QTimer.singleShot(100, self.close)

    def _open_update_settings_dialog(self) -> None:
        from pa_agent.gui.update_settings_dialog import UpdateSettingsDialog

        settings = getattr(self._ctx, "settings", None)
        if settings is None:
            return
        dialog = UpdateSettingsDialog(settings, self)
        if dialog.exec():
    ...
    def _show_previous_update_result(self) -> None:
        ...
        if result is None:
            return
        version = str(result.get("version") or "新版本")[:64]
        if result.get("ok") is True:
            self._status_bar.showMessage(f"VerdictQuant {version} 更新成功", 10000)
            return
        message = str(result.get("message") or "未知错误")[:2000]
        QMessageBox.warning(
            self,
            "自动更新未完成",
            f"更新器没有完成全部步骤，现有程序仍可继续使用。\n\n{message}",
        )
    ...
    def _on_startup_api_key_check(self) -> None:
        """Surface missing credentials in the existing banner, never a modal."""
        self._refresh_api_key_ui_state()
    ...
    def _refresh_api_key_ui_state(self) -> None:
        ...
        if cur in ("就绪", "") or "API Key" in cur or "提交分析已锁定" in cur:
            status_bar.showMessage("未配置 API Key：请点击左上角「AI 模型」填写后才能分析")
