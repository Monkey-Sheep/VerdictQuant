...
def _draw_corner_brackets(painter: QPainter, rect: QRectF, color: QColor) -> None:
    ...
    painter.drawLine(QPointF(x0 + 8, y0), QPointF(x0 + 8 + corner_length, y0))
    painter.drawLine(QPointF(x0, y0 + 8), QPointF(x0, y0 + 8 + corner_length))
    painter.drawLine(QPointF(x1 - 8 - corner_length, y0), QPointF(x1 - 8, y0))
    painter.drawLine(QPointF(x1, y0 + 8), QPointF(x1, y0 + 8 + corner_length))
    painter.drawLine(QPointF(x0 + 8, y1), QPointF(x0 + 8 + corner_length, y1))
    painter.drawLine(QPointF(x0, y1 - 8 - corner_length), QPointF(x0, y1 - 8))
    painter.drawLine(QPointF(x1 - 8 - corner_length, y1), QPointF(x1 - 8, y1))
    painter.drawLine(QPointF(x1, y1 - 8 - corner_length), QPointF(x1, y1 - 8))
...
class DecisionFlowVizPanel(QWidget):
    ...
    def _on_play_tick(self) -> None:
        if self._shutting_down or not self._play_active or not self._play_points:
            return
    ...
    def shutdown(self) -> None:
        """Stop scene activity before Qt destroys graphics items and painters."""
        if self._shutting_down:
            return
