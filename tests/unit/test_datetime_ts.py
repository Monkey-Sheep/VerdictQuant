...
def test_tradingview_naive_timestamp_is_interpreted_as_local_wall_time() -> None:
    from pa_agent.data.tradingview import _row_ts_ms

    local = datetime.now().replace(second=0, microsecond=0)
    row = SimpleNamespace(datetime=local)

    assert _row_ts_ms(row) == int(local.timestamp() * 1000)


def test_tradingview_pandas_timestamp_is_interpreted_as_local_wall_time() -> None:
    pd = pytest.importorskip("pandas")
    from pa_agent.data.tradingview import _row_ts_ms

    local = datetime.now().replace(second=0, microsecond=0)
    row = SimpleNamespace(datetime=pd.Timestamp(local))
