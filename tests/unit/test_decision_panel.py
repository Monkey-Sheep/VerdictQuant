"""Unit tests for decision and future-trend panel rendering (T18)."""
...
def test_panel_neutral_renders_yellow(panel: FutureTrendPanel):
    ...
    inner = {**data["decision"], "next_bar_prediction": data["next_bar_prediction"]}
    panel.set_prediction(inner)
    assert "中 55%" in panel._bar_direction_label.text()
    assert "#e6b800" in panel._bar_direction_label.styleSheet()


def test_panel_bearish_range_trend_shows_biased_sideways(qapp):
    ...
    decision_panel.show()
    qapp.processEvents()
    decision_panel.set_decision(
        data["decision"], diagnosis_summary=data["diagnosis_summary"]
    )
    assert "震荡偏空" in decision_panel._trend_label.text()
    assert "下跌交易区间" in decision_panel._cycle_label.text()
    assert "#f85149" in decision_panel._trend_label.styleSheet()
    decision_panel.close()
    decision_panel.deleteLater()
    qapp.processEvents()
