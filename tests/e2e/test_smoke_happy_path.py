...
def _make_ctx(tmp_path, stage2_response=None):
    ...
    ctx.settings.provider.api_key = "test-api-key"
...
def test_happy_path_shows_trading_decision(qtbot, tmp_path):
    ...
    # DecisionPanel should show a terminal decision.  The continuity guard may
    # correctly turn the mocked bullish proposal into "不下单" when the fixture
    # bars themselves are neutral.
    conclusion_text = window._decision_panel._conclusion_label.text()
    assert conclusion_text not in {"", "—", "等待分析"}
