...
本功能为未发布变更。用户选择软件手动查看后，对话定时监控应保持暂停，不因启动软件自动恢复。

## 使用

1. 打开 VerdictQuant 的“综合研究中心 → 组合监控”，或使用 `VerdictQuant.exe --monitor` 直接打开监控窗口。
2. 打开时只显示上次保存的结果。点击“刷新监控”才请求公开数据；刷新中不能重复提交，失败保留旧结果。
